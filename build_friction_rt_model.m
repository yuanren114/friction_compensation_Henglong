...
% - F0 is updated slowly in batches, not sample-by-sample.
% - the adaptation does not use raw instantaneous residual directly.
% - a short window of trusted samples is accumulated and reduced into a
%   batch error before F0 is updated.
...
%   build_friction_rt_model('friction_rt_online_generated')
...
add_block('simulink/Sources/Clock', [model_name '/Clock'], ...
    'Position', [layout.in_x layout.temp_y+50 layout.in_x+30 layout.temp_y+80]);

% Keep top-level replay helpers similar to the aa05586 hand-tuned model.
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace'], ...
    'VariableName', 'Angle_sig', ...
    'Position', [layout.ws_x layout.tas_y-5 layout.ws_x+90 layout.tas_y+25]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace1'], ...
    'VariableName', 'Omega_sig', ...
    'Position', [layout.ws_x layout.motor_y-5 layout.ws_x+90 layout.motor_y+25]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace2'], ...
    'VariableName', 'Torque_Hand_sig', ...
    'Position', [layout.ws_x layout.omega_y-5 layout.ws_x+90 layout.omega_y+25]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace3'], ...
    'VariableName', 'Torque_Motor_sig', ...
    'Position', [layout.ws_x layout.const0_y+250 layout.ws_x+90 layout.const0_y+280]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace4'], ...
    'VariableName', 'AimiCurrent_sig', ...
    'Position', [layout.ws_x layout.const0_y+300 layout.ws_x+90 layout.const0_y+330]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace5'], ...
    'VariableName', 'Ts_runtime_sig', ...
    'Position', [layout.ws_x layout.const0_y+350 layout.ws_x+90 layout.const0_y+380]);
add_block('simulink/Sources/From Workspace', [model_name '/From Workspace6'], ...
    'VariableName', 'Tfilter_runtime_sig', ...
    'Position', [layout.ws_x layout.const0_y+400 layout.ws_x+90 layout.const0_y+430]);
...
add_block('simulink/Sinks/Out1', [model_name '/count_valid_out'], ...
    'Position', [layout.out_x 960 layout.out_x+30 980]);
add_block('simulink/Sinks/Out1', [model_name '/win_elapsed_out'], ...
    'Position', [layout.out_x 1020 layout.out_x+30 1040]);

add_block('simulink/Sinks/Display', [model_name '/Display'], ...
    'Position', [layout.out_x+80 690 layout.out_x+150 720]);
end
...
add_block('simulink/Sinks/Out1', [sub '/count_valid_out'], ...
    'Position', [360 318 390 338]);
add_block('simulink/Sinks/Out1', [sub '/win_elapsed_out'], ...
    'Position', [360 378 390 398]);
...
add_line(sub, 'FilterError/5', 'count_valid_out/1');
add_line(sub, 'FilterError/6', 'win_elapsed_out/1');
end
...
% Replay-oriented main path: drive the model from From Workspace blocks.
...
add_line(model_name, 'F0_State/1', 'Display/1');

add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope'], ...
    'Position', [1180 250 1210 330], ...
    'NumInputPorts', '8');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope1'], ...
    'Position', [1240 80 1270 110], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope2'], ...
    'Position', [1240 130 1270 160], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope3'], ...
    'Position', [1240 180 1270 210], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope4'], ...
    'Position', [1240 230 1270 260], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope5'], ...
    'Position', [1240 280 1270 310], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope7'], ...
    'Position', [1240 330 1270 360], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope8'], ...
    'Position', [1240 380 1270 410], 'NumInputPorts', '1');
add_block('simulink/Sinks/Scope', [model_name '/DiagnosticsScope9'], ...
    'Position', [1240 430 1270 460], 'NumInputPorts', '1');
...
add_line(model_name, 'Error_Preprocess/5', 'DiagnosticsScope/7');
add_line(model_name, 'Error_Preprocess/6', 'DiagnosticsScope/8');
add_line(model_name, 'F0_State/1', 'DiagnosticsScope1/1');
add_line(model_name, 'Residual_Calculation/1', 'DiagnosticsScope2/1');
add_line(model_name, 'Analytic_Friction_Model/1', 'DiagnosticsScope3/1');
add_line(model_name, 'Error_Preprocess/2', 'DiagnosticsScope4/1');
add_line(model_name, 'Error_Preprocess/3', 'DiagnosticsScope5/1');
add_line(model_name, 'Error_Preprocess/4', 'DiagnosticsScope7/1');
add_line(model_name, 'Error_Preprocess/5', 'DiagnosticsScope8/1');
add_line(model_name, 'Error_Preprocess/6', 'DiagnosticsScope9/1');
end
...
"% Batch F0 adaptation using a windowed magnitude-domain error." newline ...
"% F0 is a magnitude baseline, so the update does not depend on omega sign." newline ...
...
"% Build a reliable batch adaptation error." newline ...
"% 1) compute signed mismatch e_raw for diagnostics" newline ...
"% 2) compute magnitude-domain mismatch for F0 adaptation" newline ...
"% 3) collect trusted samples in a time window" newline ...
"% 4) update only once per window using the window average" newline ...
